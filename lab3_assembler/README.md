...
The sizes of int and long are different. Thus when we put them in memory, their needed memory doubled. 
They use different sizes of registers. For example, rax is the 64-bit, "long" size register, while eax
is the 32-bit "int" size register
Additionally, the number before (%register) means the address/offsets from the registers.
...
First of all, we need to use stack pointer because we need to call other functions.
Second, there is no $ in swap function since we only deal with pointers in this function.
Additionally, a call is just an unconditional GOTO that pushes the next address on the stack, so a RET instruction can later pop it off and keep going where the call left off.(L10 slides30)
When a call instruction is executed, control passes to specific function.(cs.brown.edu/courses/cs033/docs/guides/x64_cheatsheet.pdf)
...
The last element locates at -16, which means the offsets from the array and other things are 16.
Each element takes up 4 bytes so the first element locates at -1616.
The offsets between each elements are 4 bytes since we are dealing with an  int array.
...
When we deal with dynamic arrays, we need to call malloc and we always start at the first element and add the offsets in order to assign a value to a specific slot.
When there is an assignment coming, first we need to initialize a rax, move it to the correct slot, then put the value to rax pointing to.
