...
TEST(HashtableIter, EmptyBuckets) {
  const int num_items = 20;
  Hashtable ht = CreateHashtable(6);
  ...
  ASSERT_EQ(10, NumElemsInHashtable(ht));

  ASSERT_EQ(6, ht->num_buckets);

  // Make sure there are some empty buckets
  // Assert size of each bucket
  for (int i = 0; i < ht->num_buckets; i++) {
    if (i%2 == 0) {
      ASSERT_NE(0u, NumElementsInLinkedList(ht->buckets[i]));
    } else {
      ASSERT_EQ(0u, NumElementsInLinkedList(ht->buckets[i]));
    }
  }
